...
function moveSliderIndex(index){
    ...
    if(boxs.children[0].classList.contains('0') == false){
        ...
        if(index == barConteurSlider[0]){
            boxs.style.cssText = `transition:transform .2s;transform:translate(${con * -300}px);`;
        }
        setTimeout(function(){
            for(let i =0;i<con;i++){
                boxs.removeChild(boxs.children[0]);
            };
            boxs.style.cssText = `transition:transform .2s;transform:translate(${arrSlider[index]}px);`;
